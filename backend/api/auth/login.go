...
// swagger:route GET /auth/test-login auth getTestLogin
//
// Summary: Test mode login handler
//
// Description: Allows setting precanned session tokens for testing.
//
// Parameters:
//   + name: session
//     in: query
//     description: Precanned session mode (valid, invalid, clear)
//     required: false
//     type: string
//
// Responses:
//
//	307: description: Temporary Redirect
func HandleTestLogin(database db.Db) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		if !isTestMode() {
			http.Error(w, "Test mode disabled", http.StatusForbidden)
			return
		}

		sessionType := r.URL.Query().Get("session")
		switch sessionType {
		case "valid":
		...
		case "invalid":
			http.Redirect(w, r, "/auth-error?reason=invalid_session", http.StatusTemporaryRedirect)
...
func HandleCallback(db db.Db) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
